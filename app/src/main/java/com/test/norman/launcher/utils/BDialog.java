...
public class BDialog extends Dialog {
    ...
    private blankButton bt_positive, bt_negative;
    private blankImageButton bt_cancel;
    ...
    public static BDialog newInstance(BDB bdb) {
        ...
        if (bdb.blankActivityToAutoDismiss != null) {
            bdb.blankActivityToAutoDismiss.autoDismiss(bDialog);
        }
        return bDialog;
    }
    ...
    public static BDialog newInstance(final @NonNull Context context,
        ...
        final BDialog blankDialogBox = new BDialog(context, title, subText, options, positive, negative, inputType, startingIndexChooser, extraView, negativeCustomText, positiveCustomText, flags);
        blankDialogBox.show();
        Window window = blankDialogBox.getWindow();
        window.setLayout(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
        window.setBackgroundDrawable(new ColorDrawable(android.graphics.Color.TRANSPARENT));
        window.addFlags(WindowManager.LayoutParams.FLAG_DIM_BEHIND); // This flag is required to set otherwise the setDimAmount method will not show any effect
        window.setDimAmount(DIM_LEVEL);
        return blankDialogBox;
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!containFlag(FLAG_NOT_CANCELABLE)) {
            bt_cancel = (blankImageButton) LayoutInflater.from(context).inflate(R.layout.bald_dialog_box_close_button, container, false);
        ...
        if (containFlag(FLAG_OPTIONS)) {
            ...
            final blankMultipleSelection blankMultipleSelection = (blankMultipleSelection) LayoutInflater.from(context).inflate(R.layout.bald_dialog_box_multiple_selection_view, ll, false);
            blankMultipleSelection.setOrientation(LinearLayout.HORIZONTAL);
            for (final CharSequence option : options)
                blankMultipleSelection.addSelection(option);
            blankMultipleSelection.setSelection(startingIndexChooser.chooseStartingIndex());
            ...
            ll.addView(blankMultipleSelection, 2, layoutParams);
            if (containFlag(FLAG_POSITIVE))
                bt_positive.setOnClickListener(v -> {
                    if (positive == null)
                        cancel();
                    else if (positive.activate(blankMultipleSelection.getSelection()))
                        cancel();
            ...
            if (containFlag(FLAG_NEGATIVE))
                bt_negative.setOnClickListener(v -> {
                    if (negative == null)
                        cancel();
                    else if (negative.activate(blankMultipleSelection.getSelection()))
                        cancel();
                });
        }
    }
