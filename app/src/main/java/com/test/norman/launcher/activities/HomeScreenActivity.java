...
import com.test.norman.launcher.utils.blankHomeWatcher;
...
import com.test.norman.launcher.utils.PageTransformers;
import com.test.norman.launcher.utils.S;
import com.test.norman.launcher.utils.UpdatingUtil;
import com.test.norman.launcher.views.blankImageButton;
import com.test.norman.launcher.views.BatteryView;
...
import static com.test.norman.launcher.services.NotificationListenerService.NOTIFICATIONS_ALOT;
import static com.test.norman.launcher.services.NotificationListenerService.NOTIFICATIONS_HOME_SCREEN;
import static com.test.norman.launcher.services.NotificationListenerService.NOTIFICATIONS_NONE;
import static com.test.norman.launcher.services.NotificationListenerService.NOTIFICATIONS_SOME;

public class HomeScreenActivity extends blankActivity {
    ...
    public blankPagerAdapter blankPagerAdapter;
    ...
    private blankImageButton notificationsButton, sosButton, soundButton, flashButton;
    private AudioManager audioManager;
    private blankHomeWatcher blankHomeWatcher;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        try {
            ...
            e.printStackTrace();
            blankToast.from(this).setType(blankToast.TYPE_ERROR).setText("Could not start Notification Listener Service!").show();
        }
        ...
        theme.resolveAttribute(R.attr.blank_decoration_on_background, typedValue, true);
        ...
        batteryView.setOnClickListener((v) -> blankToast.from(this)
                .setText(batteryView.percentage + "%")
                .setBig(true)
                .setType(blankToast.TYPE_INFORMATIVE)
                .show());
        blankPrefsUtils = blankPrefsUtils.newInstance(this);
        viewPagerHandler();
        blankHomeWatcher = new blankHomeWatcher(this, this::updateViewPager);
    ...
    private void viewPagerHandler() {
        blankPagerAdapter = new blankPagerAdapter(this);
        ...
        viewPagerHolder.setViewPagerAdapter(blankPagerAdapter);
        viewPagerHolder.setCurrentItem(blankPagerAdapter.startingPage);
    }

    /**
     * Updates {@link HomeScreenActivity#blankPagerAdapter} apps
     * Sets the page to {@link blankPagerAdapter#startingPage}
     */
    private void updateViewPager() {
        blankPagerAdapter.obtainAppList();
        viewPagerHolder.setCurrentItem(blankPagerAdapter.startingPage);
    ...
    static class UpdateApps extends AsyncTask<Context, Void, Void> {
        ...
        protected Void doInBackground(Context... contexts) {
            try {
                ...
                blankToast.from(contexts[0].getApplicationContext()).setType(blankToast.TYPE_ERROR).setLength(Toast.LENGTH_LONG).setText(e.getMessage());
            }
            return null;
        }
