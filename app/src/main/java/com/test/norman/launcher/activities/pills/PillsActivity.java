...
public class PillsActivity extends blankActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        recyclerView.addItemDecoration(
                new blankGridItemDecoration(r.getDimensionPixelSize(R.dimen.divider),
                        numberOfAppsInARow,
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == ADD_REMINDER_REQUEST_CODE)
            if (resultCode == RESULT_OK) {
                blankToast.from(this).setText(R.string.pill_added).show();
                refreshViews();
            }
...
//                    stringBuilder.append(' ');
                    for (int day : D.Days.ARRAY_ALL)
                        if ((reminder.getDays() & day) == day) {
                            stringBuilder.append(PillsActivity.this.getString(S.blankdayToStringId(day)));
                            stringBuilder.append(", ");
                        }
