...
import static com.test.norman.launcher.activities.PermissionActivity.EXTRA_INTENT;

/**
 * the parent of all of the activitys in this app.
 */
public abstract class blankActivity extends AppCompatActivity implements SensorEventListener {
     ...
     */
    public static boolean checkPermissions(blankActivity activity, final int requiredPermissions) {
        ...
        if ((requiredPermissions & PERMISSION_REQUEST_INSTALL_PACKAGES) != 0) {
            return
                    Build.VERSION.SDK_INT < Build.VERSION_CODES.O ||
                            (!BuildConfig.FLAVOR.equals("blankUpdates")) ||
     ...
     */
    static boolean defaultDialerGranted(blankActivity activity) {
        return true;
    }

    static boolean writeSettingsGranted(blankActivity activity) {
    ...
    static boolean notificationListenerGranted(blankActivity activity) {
    ...
    /**
     * be sure {@link #setContentView(int)} was called, and the rootview contains a {@link blankTitleBar} whose id is R.id.blank_title_bar.
     *
     * @param index the youtube video index. see
     */
    protected void setupYoutube(int index) {
        final View blankTitleBar = findViewById(R.id.blank_title_bar);
        if (blankTitleBar instanceof blankTitleBar) {
            ((blankTitleBar) blankTitleBar).getBt_help()
                    .setOnClickListener(
                            (v) -> startActivity(
